...
## What is Regression Analysis? {#Sec11}
...
the data, using either numerical or graphical summary measures. These summary measures can then be linked to a theoretical representation, or model, of the data. With a model that is calibrated by data, statements about the world can be made.
...
* Other applied statistical methods focus on managing and predicting data. *Process control*  deals with monitoring a process over time and deciding when intervention is most fruitful. Process control helps manage
...
Table \@ref(tab:Tab11) shows that much of the information concerning the height of an adult child can be attributed to, or 'explained,' in terms of the parents' height. Thus, we use the term *explanatory variable*  for measurements that provide information about a variable of interest. Regression analysis is a method to quantify the relationship between a variable of interest and explanatory variables. The methodology used to study the data in Table \@ref(tab:Tab11) can also be used to study actuarial and other risk management problems, the thesis of this book.
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_2wqpdige&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_jagstqlw%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="Section 1.1. What is regression?"></iframe>

</center>

## Fitting Data to a Normal Distribution {#Sec12}

Historically, the normal distribution had a pivotal role in the development of regression analysis. It continues to play an important role, although we will be interested in extending regression ideas to highly  'non-normal' data.
...
This curve is a probability density function with the whole real line as its domain. From equation \@ref(eq:eq11), we see that the curve is symmetric about $\mu$ (the mean and median). The degree of peakedness is controlled by the parameter $\sigma ^{2}$. These two parameters, $\mu$ and $\sigma ^{2}$, are
known as the *location*  and *scale parameters*, respectively. Appendix A3.1 provides additional details about this curve, including a graph and tables of its cumulative distribution that we will use throughout the text.

The normal curve is also depicted in Figure \@ref(fig:Fig11), a display of a now out-of-date German currency note, the ten Deutsche Mark. This note contains the image of German Carl Gauss, an eminent mathematician whose name is often linked with the normal curve (it is sometimes referred to as the *Gaussian curve*). Gauss
...
procedures for handling mixtures of 'typical' claims and others from providers who reported claims fraudulently. For this sample, we consider only those typical claims, ignoring the potentially fraudulent ones.


Table \@ref(tab:Tab12) provides several statistics that summarize different aspects of the distribution. Claim amounts are in units of logarithms of thousands of dollars. The average logarithmic claim is 0.481, corresponding to \$1,617.77 (=1000 $\exp(0.481)$). The smallest and largest claims are -3.101 (45 dollars) and 3.912 (50,000 dollars), respectively.

```{r Tab12, echo = FALSE}

# INSTALL CASdatasets - from http://cas.uqam.ca/
#install.packages("sp")
#install.packages("xts")
#install.packages("zoo")
# Please make sure that xts, sp, zoo packages are installed. Use this R repository within R via
# install.packages("CASdatasets", repos = "http://cas.uqam.ca/pub/", type="source")
# Load CASdatasets
library(CASdatasets)
data(usmassBI)
injury1<-subset(usmassBI, providerA != 0 )
...
TableOut <- cbind(length(LOGCLAIMS), mean(LOGCLAIMS),
                  quantile(LOGCLAIMS,0.5), sd(LOGCLAIMS),
                  min(LOGCLAIMS), max(LOGCLAIMS),
                  quantile(LOGCLAIMS,0.25), quantile(LOGCLAIMS,0.75)
                  )
...
TableGen1(TableData=TableOut, 
         TextTitle='Summary Statistics of Massachusetts Automobile Bodily Injury Claims',
...
For completeness, here are a few definitions. The *sample* is the set of data available for analysis, denoted by $y_1,\ldots,y_n$. Here, $n$ is the number of observations, $y_1$ represents the first observation, $y_2$ the second, and so on up to $y_n$ for the $nth$ observation. Here are a few important summary statistics.


:::: {.blackbox }
**Basic Summary Statistics**

* The *mean* is the average of observations, that is, the sum of the observations divided by the number of units. Using algebraic notation, the mean is
$$
\overline{y}=\frac{1}{n}\left( y_1 + \cdots + y_n \right) =
\frac{1}{n} \sum_{i=1}^{n} y_i.
$$
* The *median* is the middle observation when the observations are ordered by size. That is, it is the observation at which 50\% are below it (and 50\% are above it).
* The *standard deviation* is a measure of the spread, or scale, of the distribution. It is computed as
$$
s_y = \sqrt{\frac{1}{n-1}\sum_{i=1}^{n}\left(
y_i-\overline{y}\right) ^{2}} .
$$
* A *percentile* is a number at which a specified fraction of the observations is below it, when the observations are ordered by size. For example, the 25th percentile is that number so that 25\% of observations are below it.
:::: 


To help visualize the distribution, Figure \@ref(fig:Fig12) displays a *histogram* of the data. Here, the height of each rectangle shows the relative frequency of observations that fall within the range given by its base. The histogram provides a quick visual impression of the distribution; it shows that the range of the data is approximately (-4,4), the central tendency is slightly greater than zero and that the distribution is roughly symmetric.


**Normal Curve Approximation.** Figure \@ref(fig:Fig12)  also shows a normal curve superimposed, using $\overline{y}$ for $\mu$ and $s_y^{2}$ for $\sigma ^{2}$. With the normal curve, only two quantities ($\mu$ and $\sigma ^{2}$) are required to summarize the
entire distribution. For example, Table \@ref(tab:Tab12) shows that 1.168 is the 75th percentile, which is approximately the 204th ($=0.75\times 272$) largest observation from the entire sample. From the equation \@ref(eq:eq11) normal distribution,
we have that $z=(y-\mu )/\sigma$ is a standard normal, of which 0.675\ is the 75th percentile. Thus, $\overline{y}+0.675s_y=$ $0.481+0.675\times 1.101=1.224$ is the 75th percentile using the normal curve approximation.
...
`r HideRCode('Fig12.Hide',"R Code to Produce Figure 1.2")`
...
statistics, numerical summary measures. The *box plot*, also known as a 'box and whiskers' plot, is one
...
`r HideRCode('Fig13.Hide',"R Code to Produce Figure 1.3")`
...
`r HideRCode('Fig14.Hide',"R Code to Produce Figure 1.4")`
...
`r HideRCode('Fig15.Hide',"R Code to Produce Figure 1.5")`
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_gj2g1kd2&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_03drpzud%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="1.2 DataNormalDistribution"></iframe>

</center>

## Power Transforms {#Sec13}

In the Section \@ref(Sec12) example, we considered claims without justifying
...
`r HideRCode('Fig16.Hide',"R Code to Produce Figure 1.6")`
...
The upper left panel of Figure \@ref(fig:Fig17) shows the original
distribution is heavily skewed to the right. The other panels in
Figure \@ref(fig:Fig17) show the data rescaled using the square
...
`r HideRCode('Fig17.Hide',"R Code to Produce Figure 1.7")`
...
## Sampling and the Role of Normality {#Sec14}
...
summarize variables; techniques that will be used repeatedly throughout
...
$t$-statistics, requires this additional assumption.

Section \@ref(Sec18) provides an explicit statement of one
...
also discusses a related result, known as an *Edgeworth approximation*, that shows that the quality of the normal
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_2ovpv8hi&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_o5wtonwn%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="1.3 PowerTransforms"></iframe>

</center>


## Regression and Sampling Designs {#Sec15}
...
variables. As noted in Section \@ref(Sec11), regression is used
extensively in many scientific fields. [Table 1.3] lists alternative terms that you may encounter as you read regression applications.
...
**Example: Rand Health Insurance Experiment**. How are medical
...
\@ref(Sec11) Galton study, social science researchers generally
...
## Actuarial Applications of Regression {#Sec16}
...
**Pricing and adverse selection.** Regression analysis can be
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_bwgovdnf&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_m0i7x2rq%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="1.4 RegrSamplingDesigns"></iframe>

</center>



## Further Reading and References {#Sec17}
...
*  Beard, Robert E.,  Teivo Pentikäinen and Erkki Pesonen (1984).
*Risk Theory: The Stochastic Basis of Insurance* (Third
...
*  Faraway, Julian J. (2005). *Linear Models in R.* Chapman \&
Hall/CRC, New York.
*  Hogg, Robert V. (1972). On statistical education. *The
...
randomized experiment. *American Economic Review* 77, No. 3,
251-277.
...
*The American Statistician* 44, 223-230.
*  Stigler, Steven M. (1986). *The History of Statistics:  The
Measurement of Uncertainty before 1900*. The Belknap Press of
Harvard University Press, Cambridge, MA.
*  Weisberg, Sanford (2005). *Applied Linear Regression, Third
...
1.1 **MEPS health expenditures.** This exercise considers data from the Medical Expenditure Panel Survey (MEPS), conducted by the
...
1.2 **Nursing Home Utilization.** This exercise considers nursing home data provided by the Wisconsin
...
[http://dhs.wisconsin.gov] for more information.
...
**Part 2:** Use cost report year 2001 data and repeat parts (a)
and (c).

***

1.3 **Automobile Insurance Claims.** As an actuarial analyst,
...
1.5 **Automobile injury insurance claims.** 
...
1.6 **Insurance Company Expenses.** 
...
1.7 **National Life Expectancies.** Who is doing health care right?
...
several explanatory variables, are listed in [Table 1.4]. From this table, you will note that
...
UNLife <- read.csv("CSVData/UNLifeExpectancy.csv", header=TRUE)

UNLifeA <- UNLife[c("BIRTHATTEND", "FEMALEBOSS", "FERTILITY", "GDP",
                    "HEALTHEXPEND", "ILLITERATE", "PHYSICIAN", "POP",
                    "PRIVATEHEALTH", "PUBLICEDUCATION", "RESEARCHERS",
                    "SMOKING", "LIFEEXP")]
# Summarize the data
sumVar <- function(Variable){
       aggregate(Variable ~ 1, data = UNLifeA, na.action=NULL, 
      FUN = function(x) c(count = sum(is.na(x)), mean = mean(x, na.rm=TRUE), 
                          med = median(x, na.rm=TRUE), sd = sd(x, na.rm=TRUE),
                          min = min(x, na.rm=TRUE), max = max(x, na.rm=TRUE))
       ) }

TableLife <- matrix(0, 13, 6)
for (trow in 1:13) {
 TableLife[trow,] <- sumVar(UNLifeA[,trow])$Variable}
#  TableLife
rownames(TableLife) <- colnames(UNLifeA)
...
Table 1.4. **Life Expectancy, Economic and Demographic Characteristics of 185 Countries** 
...
*Source:* United Nations Human Development Report, available
at http://hdr.undp.org/en/ . 

## Technical Supplement - Central Limit Theorem {#Sec19}
...
:::: {.blackbox }
*Central Limit Theorem.* Suppose that $y_1,\ldots,y_n$ are
...
Pentikäinen and Pesonen (1984) for further discussion of
Edgeworth approximations in actuarial science. An alternative (used
in this book) that we saw in Section  \@ref(Sec13) is to transform the data, thus achieving approximate symmetry. As suggested by the Edgeworth approximation theorem, if our parent population is close to
