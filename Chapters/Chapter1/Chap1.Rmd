...
\index{graphs|see{plots}}
\index{multicollinearity|see{collinearity}}
...
\marginparjed{Statistics is about the collection, summarization and analysis of data to make statements about the real world.}
...
\marginparjed{Regression analysis is a method to quantify the relationship between a variable of interest and explanatory variables.}

\index{explanatory variable}

Table \@ref(tab:GaltonTable) shows that much of the information concerning the height of an adult child can be attributed to, or 'explained,' in terms of the parents' height. Thus, we use the term *explanatory variable*  for measurements that provide information about a variable of interest. Regression analysis is a method to quantify the relationship between a variable of interest and explanatory variables. The methodology used to study the data in Table \@ref(tab:GaltonTable) can also be used to study actuarial and other risk management problems, the thesis of this book.
...
\marginparjed{Appendix A3.1 provides additional details about the normal curve, including a graph and distribution table.}

This curve is a probability density function with the whole real line as its domain. From equation \@ref(eq:Normalpdf), we see that the curve is symmetric about $\mu$ (the mean and median). The degree of peakedness is controlled by the parameter $\sigma ^{2}$. These two parameters, $\mu$ and $\sigma ^{2}$, are
...
\index{datasets!Massachusetts bodily injury claims}
\ecaptionjed{Massachusetts Bodily Injury Claims} 
...
\bigskip
\index{symbols!$\bar{y}$, sample mean}
\index{symbols!$s_y$, sample standard deviation} 
\index{symbols!$n$, sample size}
\index{symbols!$y$, observed variable, typically the outcome of interest}
...
entire distribution. For example, Table \@ref(tab:Table12) shows that 1.168 is the 75th percentile, which is approximately the 204th ($=0.75\times 272$) largest observation from the entire sample. From the equation \@ref(eq:Normalpdf) normal distribution,
...
\index{plots!quantile-quantile, $qq$}
...
\index{model assumptions}
\index{symbols!$\mu$, population mean}
\index{symbols!$\sigma^2$, population variance}
\index{symbols!$\sigma$, population standard deviation}
\index{symbols!E , expectation operator}
\index{symbols!Var , variance operator}
...
\index{explanatory variable!independent variable}
\index{explanatory variable!regressor}
\index{explanatory variable!predictor}
\index{explanatory variable!right-hand side}
\index{explanatory variable!exogenous}
\index{dependent variable}
\index{dependent variable!outcome of interest}
\index{dependent variable!endogenous}
\index{dependent variable!response}
\index{dependent variable!regressand}
\index{dependent variable!left-hand side}
\index{dependent variable!explained}
...
\linejed 
\index{examples!Rand health insurance experiment}
...
\index{datasets!MEPS health expenditures}
...
\index{datasets!automobile insurance claims}
...
\index{datasets!automobile injury insurance claims}
\index{actuarial \& financial terms and concepts!closed claim}
...
\index{datasets!national life expectancies}
...
at http://hdr.undp.org/en/ . \scalefont{1.25}
...
\index{theorems!central limit}
\index{symbols!$\Phi(\cdot)$, standard normal distribution function}
...
\bigskip
\index{symbols!$\Pr$, probability operator}
