...
\index{supuestos del modelo!representación observable}
\index{símbolos!$\beta_0$, intercepto de regresión (poblacional)}
\index{símbolos!$\beta_1$, coeficiente de regresión (poblacional) asociado con $x_1$}
...
\text{E3}. & \mathrm{E}~\varepsilon _i=0 \text{ y } \mathrm{Var}~\varepsilon _i=\sigma ^{2}. \\
...
text(0.6, 3.5, "Línea de Regresión \n Verdadera Desconocida", cex=.9)
...
desviaciones $\varepsilon _i$, entonces una estimación deseable de $\sigma
^2$ sería $(n-1)^{-1}\sum_{i=1}^{n}\left( \varepsilon
_i-\overline{\varepsilon }\right) ^2$. Como $\{\varepsilon _i\}$
...
Comparando las definiciones de $s^2$ y $(n-1)^{-1}\sum_{i=1}^{n}\left( \varepsilon _i-\overline{\varepsilon }\right) ^2$, verá dos diferencias importantes. Primero, al definir $s^2$ no hemos restado el residuo promedio de cada
...
Comenzaremos con la representación del error de la Sección 2.2. Bajo este conjunto de suposiciones, las desviaciones \{$\varepsilon _i$\} son
...
aproximaciones para las desviaciones (no observadas) \{$\varepsilon _i$\}.
...
*  **Falta de Independencia**. Puede haber relaciones entre las desviaciones \{$\varepsilon _i$\} de modo que no sean independientes.
...
mtext("Cuantiles Muestrales", side=2,at=20500,las=1,cex=1.1, adj=.5)
qqnorm(residuals(model.Kenosha), main="", ylab="", las=1)
mtext("Cuantiles Muestrales", side=2,at=9050,las=1,cex=1.1, adj=.5)
...
A continuación, examinamos los datos a lo largo del tiempo, como se muestra gráficamente en la Figura \@ref(fig:Fig210). Estos son gráficos de dispersión de los rendimientos versus el tiempo, llamados *gráficos de series temporales*. En la Figura \@ref(fig:Fig210), se puede ver claramente

 el rendimiento más bajo del mercado y un vistazo rápido al eje horizontal revela que este punto inusual está en octubre de 1987, el momento del conocido colapso del mercado.
...
mtext("Rendimiento Mensual", side=2, at=.38,las=1,cex=1.1, adj=.5)
...
text(0.16, 0.02, "PUNTOS ATÍPICOS DE 1990", cex=0.8)
...
b. Considera el modelo $y_i=\beta_1 z_i^2 + \varepsilon _i$, un modelo cuadrático que pasa por el origen. Utiliza el resultado del apartado (a) para determinar la estimación de mínimos cuadrados de $\beta_1$.
...
b. Ingresos logarítmicos y retornos. <br>
...
\end{equation*}
Estas son las expresiones usuales para la pendiente $b_1$ (Ejercicio 2A.8) y el intercepto $b_0$.
