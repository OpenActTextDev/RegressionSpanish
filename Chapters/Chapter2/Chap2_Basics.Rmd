...
## Correlations and Least Squares {#Sec21}
...
Data sets can get complicated, so it will help if you begin by working with each variable separately. The two panels in Figure \@ref(fig:Fig21) show histograms that give a quick visual impression of the distribution of each variable in isolation of the other. Table \@ref(tab:Tab21) provides corresponding numerical summaries.  To illustrate, for the population variable (POP), we see that the area with the smallest number contained 280 people whereas the largest contained 39,098. The average, over 50 ZIP codes, was 9,311.04. For our second variable, sales were as low as 189 and as high as 33,181.


(ref:Fig21) **Histograms of Population and Sales.** Each distribution is skewed to the right, indicating that there are many small areas compared to a few areas with larger sales and populations.
...
As Table \@ref(tab:Tab21) shows, the basic summary statistics
...
(ref:Fig22) **A scatter plot of the lottery data**. Each of the 50  plotting symbols corresponds to a zip code in the study. This figure suggests that postal areas with larger populations have larger lottery revenues.
...
bounded by -1 and 1, that is, $-1\leq r\leq 1$. (Exercise 2.3
...
\@ref(fig:Fig22). Numerically, the main response is the
...
generic values. For the $i$th observation, $y_i-\left( b_0^{\ast}+b_1^{\ast}x_i\right)$ represents the deviation of the
...
Does this procedure yield a sensible line for our Wisconsin lottery sales? Earlier, we computed $r=0.886$. From this and the basic summary statistics in Table \@ref(tab:Tab21), we have $b_1 = 0.886 \left( 8,103\right) /11,098=0.647$ and $b_0 = 6,495-(0.647)9,311 = 469.7.$ This yields the fitted regression line 
$$
...
**Example: Summarizing Simulations.** Regression analysis is a tool for summarizing complex
...
presented in Figure \@ref(fig:Fig23). The scatterplot shows a strong
but not perfect relationship between the $VaR$ and the $CTE$, the
correlation coefficient turns out to be $r=0.782$.



(ref:Fig23) **Plot of Conditional Tail Expectation (CTE) versus Value at Risk (VaR).** Based on $n=1,000$ simulations from a 10-year European put bond. *Source*: Manistre and Hancock (2005).
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_ex78f58s&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_4ufzfnw4%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.1 CorrlnLeastSquares"></iframe>

</center>


## Basic Linear Regression Model  {#Sec22}
...
\text{E3}. & \mathrm{E}~\varepsilon_i=0 \text{ and } \mathrm{Var}~\varepsilon_i=\sigma ^{2}. \\
...
observed explanatory variable, $x$. Other unobserved variables that influence the measurement of $y$ are interpreted to be included in the "error" term $\varepsilon_i$, which is also known as the "disturbance" term. The independence of errors, E4, can be motivated by assuming that \{$\varepsilon_i$\} are realized through a simple random sample from an unknown population of errors.

Assumptions E1-E4 are equivalent to F1-F4. The error representation provides a useful springboard for motivating goodness of fit measures (Section \@ref(Sec23)). However, a drawback of the
error representation is that it draws the attention from the observable quantities $(x_i,y_i)$ to an unobservable quantity, \{$\varepsilon_i$\}. To illustrate, the sampling basis, viewing
\{$\varepsilon_i$\} as a simple random sample, is not directly verifiable because one cannot directly observe the sample \{$\varepsilon_i$\}. Moreover, the assumption of additive errors in E1 will be troublesome when we consider nonlinear regression models.

Figure \@ref(fig:Fig24) illustrates some of the assumptions of
...
normal curve, as in Figure \@ref(fig:Fig24).



(ref:Fig24) **The distribution of the response varies by the level of the explanatory variable.**
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_kcj99jbm&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_wjwn6agv%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.2 BasicLinearRegrModel"></iframe>

</center>



## Is the Model Useful? Some Basic Summary Measures {#Sec23}
...
### Partitioning the Variability {#Sec231}
...
explained by $x$." Figure \@ref(fig:Fig25) is a geometric
...
\@ref(fig:Fig25) with an observation below the fitted regression
line.

(ref:Fig25) **Geometric display of the deviation decomposition.** 
...
Now, from the algebraic decomposition in equation \@ref(eq:eq21), square each side of the equation and sum
...
### The Size of a Typical Deviation: *s* {#Sec232}
...
model specification in Section \@ref(Sec26). We now show
how to use the residuals to estimate $\sigma ^2$. From a first
course in statistics, we know that if one could observe the
deviations $\varepsilon_i$, then a desirable estimate of $\sigma
^2$\ would be $(n-1)^{-1}\sum_{i=1}^{n}\left( \varepsilon
_i-\overline{\varepsilon }\right) ^2$. Because \{$\varepsilon_i$\}
...
Comparing the definitions of $s^2$ and $(n-1)^{-1}\sum_{i=1}^{n}\left( \varepsilon_i-\overline{\varepsilon }\right) ^2$, you will see two important differences. First, in defining $s^2$ we have not subtracted the average residual from each
...
($df$). In particular, the mean square for errors ($MSE$) equals $s^2$ and the regression sum of squares equals the regression mean
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_ltg98a7y&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_neosfut1%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.3 IsTheModelUseful"></iframe>

</center>

## Properties of Regression Coefficient Estimators  {#Sec24}
...
estimator. Using equation \@ref(eq:eq25), we see that $se(b_1)$ is
...
relations between $y$ and $x$. Figure \@ref(fig:Fig26) illustrates
...
(ref:Fig26) **These three scatter plots exhibit the same linear relationship between $y$ and $x$.** The plot on the left exhibits greater variability about the line than the plot in the middle. The plot on the right exhibits a smaller standard deviation in $x$ than the plot in the middle.
...
Equation \@ref(eq:eq24) also implies that the regression
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_e7sbcw9f&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_fobza624%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.4 PropertiesRegressionCoeff"></iframe>

</center>

## Statistical Inference  {#Sec25}
...
### Is the Explanatory Variable Important?: The *t*-Test  {#Sec251}
...
\@ref(tab:Tab21), we have $s_x = 11,098$. Thus, the standard
...
0.647\pm (2.011)(.0488),
$$
or (0.549, 0.745). Similarly, if population differs by 1,000, a 95\% confidence interval for the expected change in sales is (549, 745). Here, we use the $t$-value $t_{48,0.975}=2.011$ because there are 48 (= 
$n$-2) degrees of freedom and, for a 95\% confidence interval, we need the 97.5th percentile.

### Prediction Intervals  {#Sec253}
...
{\small \text{prediction error} }& {\small =} & {\small  \text{error in estimating
the } }&
{\small +} & {\small \text{deviation of the additional } }\\
&  & {\small \text{regression line at }x}_{\ast} &  & {\small
\text{response from its mean}}
\end{array}
$$
...
$$
As with $se(b_1)$, the terms $n^{-1}$ and $\left( x_{\ast}-\overline{x} \right) ^2/\left[ (n-1)s_x^2\right]$ become
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_mdnibebm&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_4w3wg29v%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.5 StatisticalInference"></iframe>

</center>

## Building a Better Model: Residual Analysis {#Sec26}
...
We will begin with the Section 2.2 error representation. Under this set of assumptions, the deviations \{$\varepsilon_i$\} are
...
approximations for the (unobserved) deviations \{$\varepsilon_i$\}.
...
will be our first look at handling relationships between \{$\varepsilon_i$\} and additional explanatory variables. We have, however, already had an introduction to the effect of normal
...
**Example: Outliers and High Leverage Points.** Consider the fictitious data set of 19 points plus three points, labeled A, B, and C, given in Figure \@ref(fig:Fig27) and [Table 2.5](#Table25). Think of the first 19 points as "good" observations that represent some type of phenomena. We want to investigate the effect of adding a single aberrant point.
...
(ref:Fig27) **Scatterplot of 19 base plus three unusual points, labeled A, B and C.**
...
**Example: Lottery Sales -- Continued.** Figure \@ref(fig:Fig28) exhibits an outlier; the point in the upper
...
(ref:Fig28) **Scatter plot of SALES versus POP, with the outlier corresponding to Kenosha marked.**
...
To check the accuracy of our assumptions, it is also customary to check the normality assumption. One way of doing this is the $qq$ plot, introduced in Section 1.2. The two panels in Figures \@ref(fig:Fig29) are $qq$ plots with and without the Kenosha
zip code. Recall that points "close" to linear indicate approximate normality. In the right-hand panel of Figure \@ref(fig:Fig29), the sequence does appear to be linear so that residuals are approximately normally distributed. This is not the case in the left-hand panel, where the sequence of points appears to climb dramatically for large quantiles. The interesting thing is that the non-normality of the distribution is due to a
single outlier, not a pattern of skewness that is common to all the observations.


(ref:Fig29) **$qq$ Plots of Wisconsin Lottery Residuals.**  The left-hand panel is based on all 50 points. The right-hand panel is based on 49 points, residuals from a regression after removing Kenosha.
...
<iframe id="kaltura_player" src='https://cdnapisec.kaltura.com/p/1660902/embedPlaykitJs/uiconf_id/55063162?iframeembed=true&amp;entry_id=1_6iyr9pdp&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_j0rn1dwr%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D'  style="width: 576px;height: 324px;border: 0;" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  title="2.6 ResidualAnalysis"></iframe>

</center>


## Application: Capital Asset Pricing Model {#Sec27}
...
We begin by interpreting some basic summary statistics, in Table \@ref(tab:Tab28), in terms of financial theory. First, an
...
\@ref(tab:Tab28) is that the smallest market return, -0.22052,
...
Figure \@ref(fig:Fig210). These are scatter plots of the
returns versus time, called *time series plots*. In Figure
\@ref(fig:Fig210), one can clearly see the smallest market
...
(ref:Fig210) **Time series plot of returns from the Lincoln National Corporation and the market.** There are 60 monthly returns over the period January, 1986 through December, 1990.
...
The scatter plot in Figure \@ref(fig:Fig211) graphically
...
\@ref(fig:Fig211) and represents a high leverage point. With
the regression line (described below) superimposed, the two outlying
points that can be seen in Figure \@ref(fig:Fig210) are also
evident. Despite these anomalies, the plot in Figure
\@ref(fig:Fig211) does suggest that there is a linear
relationship between Lincoln and market returns.


(ref:Fig211) **Scatterplot of Lincoln's return versus the S\&P 500 Index return.** The regression line is superimposed, enabling us to identify the market crash and two outliers.
...
## Illustrative Regression Computer Output {#Sec28}
...
## Further Reading and References {#Sec29}
...
variables. Section \@ref(Sec27) described an important exception to
...
$y_i=\beta_1 x_i + \varepsilon_i$, that is, regression with one
...
b. Consider the model $y_i=\beta_1 z_i^2 + \varepsilon_i$, a
...
b. Follow the same steps to show 
$$
\sum_{i=1}^{n}\left( y_i - \overline{y} \right) \left( x_i-\overline{x}\right) 
=\sum_{i=1}^{n} x_i y_i - n \overline{x}~\overline{y}.
$$
c. Show that
$$
b_{1}=
\frac{\sum_{i=1}^{n} \left( y_i-\overline{y}\right) \left( x_i- \overline{x}\right) }
...
\end{array}
$$
b. Table 2.11 summarizes a regression using logarithmic revenues and returns.<br> 
...
revenues are $e^2$ = 7.389 millions of dollars).<br>
...
in Figure \@ref(fig:Fig212). Fit a linear regression model of $LIFEEXP$ using the explanatory variable $x=FERTILITY$.


(ref:Fig212) **Plot of FERTILITY versus LIFEEXP.** 
...
## Technical Supplement - Elements of Matrix Algebra  {#Sec211}
...
### Some Special Matrices   {#Sec2112}


*   A *square matrix* is a matrix where the number of rows equals the number of columns, that is, $n=k$.

*   The *diagonal numbers* of a square matrix are the numbers of a matrix where the row number equals the column number, for example, $a_{11}$, $a_{22}$, and so on. A *diagonal matrix* is a square matrix where all non-diagonal numbers are equal to 0. For example,
$$
\mathbf{A}=\left(
\begin{array}{ccc}
-1 & 0 & 0 \\
...
*   A *symmetric matrix* is a square matrix $\mathbf{A}$ such that the matrix remains unchanged if we interchange the roles of the rows and columns. More formally, a matrix $\mathbf{A}$ is symmetric if $\mathbf{A=A} ^{\prime }$. For example,
$$
\mathbf{A}=\left(
\begin{array}{ccc}
1 & 2 & 3 \\
...
model, $\mathbf{y=X } \boldsymbol \beta + \boldsymbol \varepsilon.$ Other useful quantities include
$$
...
$$
These are the usual expressions for the slope $b_1$ (Exercise 2.8)
and intercept $b_0$.

***

### Random Matrices   {#Sec2114}
...
From equation \@ref(eq:eq210), it can be shown that
\begin{equation}
...
(\#eq:eq211) 
\end{equation}
For example, if $\mathbf{A} = (a_1, a_2, \ldots,a_n)= \mathbf{a}^{\prime}$ and **B = 0**, then equation \@ref(eq:eq211) reduces to
$$
