...
\hline p-valor & \Pr(t_{n-(k+1)}>t-\mathrm{ratio}) &
\Pr(t_{n-(k+1)}<t-\mathrm{ratio}) & \Pr(|t_{n-(k+1)}|>|t-\mathrm{ratio}|) \\
\end{array}
\\ 
\begin{array}{ll}\hline
\textit{Notas:} &
...
$$
    \text{Tabla 3.11: Características de lotería, económicas y demográficas } \\
        \text{ de cincuenta códigos postales de Wisconsin}
$$
$$
\small{
\begin{array}{ll}
    ...
    \text{MEDHVL} & \text{Valor medianos de viviendas en 1000s} \\ 
         & ~~~ \text{ para viviendas ocupadas por sus propietarios} \\ 
