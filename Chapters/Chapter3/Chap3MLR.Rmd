...
```{r ref.label = 'FaceSumStats', eval = FALSE}
```

```{r ref.label = 'TermLifeTwoPlots', eval = FALSE}
```
...
```{r TermLifeSMatrix, fig.cap='(ref:TermLifeSMatrix)', echo=FALSE, fig.align='center', out.width = "80%", fig.asp=1.1}
...
```{r ref.label = 'TermLifeCorr', eval = FALSE}
```

```{r ref.label = 'TermLifeSMatrix', eval = FALSE}
```
...
Consider the question: "Can knowledge of education, household size, and income help us understand the demand for insurance?" The correlations in Table \@ref(tab:TermLifeCorr) and the graphs in Figures \@ref(fig:TermLifeTwoPlots) and \@ref(fig:TermLifeSMatrix) suggest that each variable, EDUCATION, NUMHH, and LNINCOME, may be a useful explanatory variable of LNFACE when taken individually. It seems reasonable to investigate the *joint* effect of these variables on a response.
...
To illustrate the calculation of the standard errors, first note that from Table \@ref(tab:ANOVATerm) we have that the residual standard deviation is $s=1.525$. Using a statistical package, we have
...
```{r ref.label = 'RefrigSumStats', eval = FALSE}
```

```{r ref.label = 'RefrigCorr', eval = FALSE}
```

</div>

A regression model was fit to the data. The fitted regression equation appears in Table \@ref(tab:RefrigFittedModel), with $s=60.65$ and $R^2=83.8\%$.

```{r RefrigFittedModel, echo=FALSE}
tableout <- data.frame(
...
```{r RefrigAddedVarPlot, fig.cap='(ref:RefrigAddedVarPlot)', fig.align='center', out.width="60%", echo = FALSE}
...
```{r ref.label = 'RefrigAddedVarPlot', eval = FALSE}
```
...
```{r LinesLetterPlot, fig.cap='(ref:LinesLetterPlot)', echo=FALSE, out.width="100%"}
Term <- read.csv("CSVData/TermLife.csv", header=TRUE)
Term2  <-subset(Term, FACE > 0)
...
Figure \@ref(fig:LinesLetterPlot) suggests that LNFACE is lower for those single than others for a given level of income. Thus, we now consider a regression model, `LNFACE = β_0 + β_1 LNINCOME + β_2 SINGLE + ϵ`. The regression function can be written as:
...
To interpret $b_2 = -0.800$, we say that we expect the logarithmic face to be smaller by 0.80 for a survey respondent who is single compared to the other category. This assumes that other things, such as income, remain unchanged. For a graphical interpretation, the two fitted regression lines are superimposed in Figure \@ref(fig:LinesLetterPlot).
...
```{r letterinteract, fig.cap='(ref:letterinteract)', echo=FALSE, out.width='100%'}
...
```{r ref.label = 'Curvilinear', eval = FALSE}
```
...
```{r ref.label = 'Charity', eval = FALSE}
```
...
The negative sign of the PUBLICEDUCATION coefficient is surprising, given that the sign of the correlation between PUBLICEDUCATION and LIFEEXP is positive and intuition suggests a positive relation. To check this result, an added variable plot appears in Figure \@ref(fig:UNLIFE2). <br>
   **d(i).** For an added variable plot, describe its purpose and a method for producing it. <br>
   **d(ii).** Calculate the correlation corresponding to the added variable plot that appears in Figure \@ref(fig:UNLIFE2). <br>
        
(ref:UNLIFE2) **Added variable plot of PUBLICEDUCATION versus LIFEEXP, controlling for FERTILITY and lnHEALTH**

```{r UNLIFE2, fig.cap='(ref:UNLIFE2)', echo=FALSE, out.width = "60%", fig.asp=1.1}
#  FIGURE 3.11
...
```{r ref.label = 'UNLIFE2', eval = FALSE}
```
