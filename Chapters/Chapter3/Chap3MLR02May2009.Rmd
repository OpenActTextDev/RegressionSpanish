...
```{r fig:TermLifeTwoPlots, fig.cap="Income versus Face Amount of Term Life Insurance. The left-panel is a plot of face versus income, showing a highly nonlinear pattern. In the right-hand panel, face versus income is in natural logarithmic units, suggesting a linear (although variable) pattern.", echo=FALSE, fig.align='center', out.width = "80%"}
# Load necessary libraries
library(png)

# Load the image
img <- readPNG("Chapter3/F3TermLifeTwoPlots.png")
...
```{r fig:TermLifeSMatrix, fig.cap="Scatterplot matrix of four variables. Each square is a scatter plot.", echo=FALSE, fig.align='center', out.width = "100%"}
# Load necessary libraries
img <- readPNG("Chapter3/F3TermLifeSMatrix.png")
...
```{r fig:3DPlane, fig.cap="An example of a three-dimensional plane.", echo=FALSE, fig.align='center', out.width = "50%"}
# Load necessary libraries
img <- readPNG("Chapter3/F33DPlane.png")
...
```{r RefrigAddedVarPlot, fig.cap="An added variable plot. The residuals from the regression of PRICE on the explanatory variables, omitting ECOST, are on the vertical axis. On the horizontal axis are the residuals from the regression fit of ECOST on the other explanatory variables. The correlation coefficient is -0.48.", fig.align='center', out.width='.6\\textwidth'}
knitr::include_graphics("Chapter3/F3RefrigAddedVarPlot.eps")
```
...
```{r fig.cap="Letter plot of LNFACE versus LNINCOME, with the letter code 'S' for single and 'o' for other. The fitted regression lines have been superimposed. The lower line is for single and the upper line is for other.", echo=FALSE, out.width="60%"}
knitr::include_graphics("Chapter3/F3LinesLetterPlot.eps")
```
...
```{r interaction, fig.cap="Plot of $\\mathrm{E}~y = \\beta_0 + \\beta_1 x_1 + \\beta_2 x_2 + \\beta_3 x_1 x_2$ versus $x_1$ and $x_2$.", echo=FALSE, out.width='60%'}
knitr::include_graphics("Chapter3/F3Interaction.png")
```
...
```{r letterinteract, fig.cap="Letter plot of LNFACE versus LNINCOME, with the letter code `S` for single and `o` for other. The fitted regression lines have been superimposed. The lower line is for single and the upper line is for other.", echo=FALSE, out.width='60%'}
knitr::include_graphics("Chapter3/F3LetterInteract.png")
```
...
```{r expenses, fig.cap="Twenty-Three Regression Coefficients from an Expense Cost Model", echo=FALSE, out.width='100%'}
knitr::include_graphics("Chapter3/T3Expenses.png")
```
...
```{r Curvilinear, fig.cap="Plot of $\\mathrm{E}~y = \\beta_0 + \\beta_1~x_1 + \\beta_2~x_2 + \\beta_{11}~x_1^2 + \\beta_{22}~x_2^2 + \\beta_{12}~x_1~x_2$ versus $x_1$ and $x_2$.", echo=FALSE, out.width='60%'}
knitr::include_graphics("Chapter3/F3Curvilinear.png")
```
...
```{r Charity, fig.cap="The marginal change in $\\mathrm{E}~y$ is lower below \\$97,500. The parameter $\\beta_2$ represents the difference in the slopes.", echo=FALSE, out.width='60%'}
knitr::include_graphics("Chapter3/F3Charity.png")
```
...
```{r Break, fig.cap="Plot of expected commissions ($\\mathrm{E}~y$) versus number of shares traded ($x$). The break at $x=100$ reflects savings in administrative expenses. The lower slope for $x \geq 100$ reflects economies of scales in expenses.", echo=FALSE, out.width='60%'}
knitr::include_graphics("Chapter3/F3Break.png")
```
...
1. Consider a fictitious data set of $n = 100$ observations with $s_y = 80$. We run a regression with three explanatory variables to get $s = 50$.
...
2. Consider a fictitious data set of $n = 100$ observations with $s_y = 80$. We run a regression with three explanatory variables to get $s = 50$. We also get
...
3. Consider the following small fictitious data set. You will be fitting a regression model to $y$ using two explanatory variables, $x_1$ and $x_2$.
