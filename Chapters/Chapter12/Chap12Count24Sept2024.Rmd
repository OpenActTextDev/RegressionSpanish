...
*logarithmic link function*, that is, $\ln
...
\beta$. From equation \@ref(eq:eq125), we see that if a row of
...
Table \@ref(tab:Tab126) summarizes a simpler fitted Poisson
model with only additive effects. Table \@ref(tab:Tab126)
...
\@ref(tab:Tab126) to help interpret the parameters. For
...
Female    <- 1*(Singapore$SexInsured == "F" )
...
\@ref(tab:Tab127) compares several fitted models, providing
...
under the insurance contract. Table \@ref(tab:Tab127) also shows
the improvement in the overall fit when including the fitted model
summarized in Table \@ref(tab:Tab126). When compared to a
...
Tab127 <- t(rbind(col1, col2, col3, col4, col5) )
Tab127[6, 1:2] <- NA
Tab127[5,2] <- 0
...
summarized in Table \@ref(tab:Tab126). The resulting
...
the goodness of fit statistics given in Table \@ref(tab:Tab127).
...
values. We interpret the findings of Table \@ref(tab:Tab127) to
