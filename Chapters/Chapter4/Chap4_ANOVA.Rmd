...
each binary variable), Section \@ref(Sec42) will introduce
tests of sets of regression coefficients.

```{r Tab43, echo=FALSE, results='asis'}
tableout <- 
  data.frame(
    Model_1_Coefficient = c(0.452, 0.205, 0.248, 3.395, -0.557, "", -0.789),
    Model_1_t_ratio = c(5.74, 5.30, 3.57, 3.77, -2.15, "", -1.59),
    Model_2_Coefficient = c(0.452, 0.205, 0.248, 3.395, 0.232, 0.789, ""),
    Model_2_t_ratio = c(5.74, 5.30, 3.57, 2.74, 0.44, 1.59, ""),
    Model_3_Coefficient = c(0.452, 0.205, 0.248, 2.838, "", 0.557, -0.232),
    Model_3_t_ratio = c(5.74, 5.30, 3.57, 3.34, "", 2.15, -0.44)
  )
...
Section \@ref(Sec472) provides the mathematical underpinnings. To understand the extra-sum-of-squares principle, recall that the error sum of squares for the full model is determined to be the minimum value of
...
summarized in Section \@ref(Sec473). In this theory, regression coefficients need not be identifiable. Alternatively, one
...
covariate. In Section \@ref(Sec43), we introduced the one
...
b. We now introduce the $REGION$ variable, summarized in Table \@ref(tab:Tab411). A boxplot of life expectancies versus $REGION$ is given in Figure \@ref(fig:Fig48). Describe what we learn from the Table and boxplot about the effect of $REGION$ on $LIFEEXP$.
...
LifeExp4$lnHEALTH <- log(LifeExp4$HEALTHEXPEND)
LifeExp4.good <- na.omit(LifeExp4) 
RegionLab <- c("Arab States","East Asia and the Pacific",
               "Latin American and the Carribean","South Asia",                                "Southern Europe","Sub-Saharan Africa",
              " Central and Eastern Europe","High Income OECD")   
...
#Totals <- c("", "All", sum(t1[,2]), round(mean(LifeExp$LIFEEXP), digits = 1) )
Totals <- c("", "All", sum(t1[,2]), 67.4)

Tab411 <- rbind(Tab411A, Totals)
colnames(Tab411) <- c("Region", "Description of the Region", "Number", "Mean")
rownames(Tab411) <- NULL

TableGen1(TableData=Tab411 , 
         TextTitle='Average Life Expectancy by Region',
                  Align='ccccc', 
         ColWidth = ColWidth4) %>%
...
**One Categorical Variable Model.** Consider the model with one categorical variable introduced in Section \@ref(Sec43) with $c$ levels of the categorical variable. From equation \@ref(eq:eq49), this model can be written as
...
(\#eq:eq422)
$$
where $\boldsymbol \alpha_1 = \boldsymbol \beta_1 + \mathbf{A}\boldsymbol \beta_2$ is a new vector of parameters. The
...
(\#eq:eq426)
$$
As noted in Section \@ref(Sec43), the quantity $(Error ~SS)_{reduced} - (Error ~SS)_{full}$ is called the *extra sum of squares*, or Type
...
Recall the general linear model from Section \@ref(Sec44). That is, we use
$$
