...
As before, the dependent variable is $y =$ LNFACE, the amount that the company will pay in the event of the death of the named insured (in logarithmic dollars). Table \@ref(tab:MaritalSumStats) summarizes the dependent variable by the level of the categorical variable. This table shows that the marital status "married" is the most prevalent in the sample and that those married choose to have the most life insurance coverage. Figure \@ref(fig:BoxFACEMARSTAT) gives a more complete picture of the distribution of LNFACE for each of the three types of marital status. The table and figure also suggest that those living together have less life insurance coverage than the other two categories.
...
(ref:BoxFACEMARSTAT) **Box Plots of Logarithmic Face, by Level of Marital Status** 
...
Although the three models in Table \@ref(tab:MSTermLifeRegrCoeff) are
...
For simplicity, here we report only results for hospital episodes. Although families were randomly assigned to plans, Keeler and Rolph (1988) used regression methods to control for participant attributes and isolate the effects of plan cost-sharing. Table \@ref(tab:RandHIECoefficients) summarizes the regression coefficients, based on a sample of $n=1,967$ episode expenditures. In this regression, logarithmic expenditure was the dependent variable.
...
It can be useful to examine several regression coefficients at the same time. For example, when assessing the effect of a categorical variable with $c$ levels, we need to say something jointly about the $c-1$ binary variables that enter the regression equation. To do this, Section \@ref(S4:SetsRegCoeff) introduces a method for handling linear combinations of regression coefficients. Section \@ref(S4:GenLinHypo) shows how to test several linear combinations, and Section \@ref(S4:SetsInference) presents other inference applications.
...
i. We begin by running a regression model with all $k+p=5$ variables. The results were reported in Table \@ref(tab:MSTermLifeANOVA), where we saw that $(Error~SS)_{full} = 615.62$ and $s_{full}^2 = (1.513)^2 = 2.289$.
...
\@ref(tab:MSTermLifeANOVA), the $F$-ratio is 68.66 / 2.29 = 29.98.
...
### Estimating and Predicting Several Coefficients {#S4:SetsInference}
...
CLASS. Table \@ref(tab:AutoSumStats) shows 18 risk classes - further
classification information is not given here to protect proprietary
interests of the insurer.

Table \@ref(tab:AutoSumStats) summarizes the results from $n=6,773$
...
Figure \@ref(fig:BoxplotAuto) shows the distribution of logarithmic
...
(ref:BoxplotAuto) **Box Plots of Logarithmic Claims by Risk Class** 
...
\end{array}
$$
The last two equalities follow from algebra manipulation. The $Factor ~SS$ plays the same role as the $Regression ~SS$ in Chapters 2 and 3. The variability decomposition is summarized in Table \@ref(tab:OneFactorANOVA).
...
appears in Table \@ref(tab:ANOVAAuto). Here, we see that the mean
...
variable, or covariate, using $y_{ij} = \beta_0 + \beta_1 x_{ij} + \varepsilon_{ij}$. Table \@ref(tab:OneFactorCovariate) summarizes
...
For each model presented in Table \@ref(tab:OneFactorCovariate),
...
Each of the five models defined in Table \@ref(tab:OneFactorCovariate)
was fit to this subset of the Hospital case study. The summary
statistics are in Table \@ref(tab:DRGModels). For this data set, there
...
Using binary variables, each of the models in Table \@ref(tab:OneFactorCovariate) can be written in a regression format. As we have seen in Section 4.2, when a model can be written as a subset of another, larger model, we have formal testing procedures available to decide which model is more appropriate. To illustrate this testing procedure with our DRG example, from Table \@ref(tab:DRGModels) and the associated plots, it seems clear that the DRG factor is important. Further, a $t$-test, not presented here, shows that the covariate $x$ is important. Thus, let's compare the full model $E[y_{ij}] = \beta_{0,j} + \beta_{1,j}x$ to the reduced model $E[y_{ij}] = \beta_{0,j} + \beta_1x$. In other words, is there a different slope for each DRG?
...
Here, the variables $x_1$, $x_2$, and $x_3$ are known as the *main effects*. Table \@ref(tab:TwoFactorModel) helps interpret this equation. Specifically, there are six types of people that we could encounter: males and females who are young, middle-aged, or old. We have six parameters in equation \@ref(eq:eq412). Table \@ref(tab:TwoFactorModel) provides the link between the parameters and the types of people. By using the interaction terms, we do not impose any prior specifications on the additive effects of each factor. From Table \@ref(tab:TwoFactorModel), we see that the interpretation of the regression coefficients in equation \@ref(eq:eq412) is not straightforward. However, using the additive model with interaction terms is equivalent to creating a new categorical variable with six levels, one for each type of person. If the interaction terms are critical in your study, you may wish to create a new factor that incorporates the interaction terms simply for ease of interpretation.
...
b. We now introduce the $REGION$ variable, summarized in Table 4.11. A boxplot of life expectancies versus $REGION$ is given in Figure \@ref(fig:UNLIFEPlot3). Describe what we learn from the Table and boxplot about the effect of $REGION$ on $LIFEEXP$.

(ref:UNLIFEPlot3) **Boxplots of LIFEEXP by REGION** 
...
## Technical Supplement - Matrix Expressions

### Expressing Models with Categorical Variables in Matrix Form
...
### Calculating Least Squares Recursively
