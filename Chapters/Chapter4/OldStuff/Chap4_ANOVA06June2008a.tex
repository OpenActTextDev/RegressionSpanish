...
It turns out that Corr(MAR0,MAR1) = -0.88, Corr(MAR0,MAR2) =-0.12
and Corr(MAR1,MAR2) = -0.37.
...
enter the regression equation. To do this, Section
\ref{S4:SetsRegCoeff} introduces the general idea of \emph{sets} of
regression coefficients. Section \ref{S4:GenLinHypo} shows
applications in the context of hypothesis testing and Section
\ref{S4:SetsInference} presents other inference applications.
...
\right) =\mathbf{d}.
\end{equation*}
From a list of $k+p$ variables $x_1,...,x_{k+p}$, you may drop any
$p$ that you deem appropriate. The additional variables do not need
to be the last $p$ in the regression specification. Dropping
$x_{k+1},...,x_{k+p}$ is for notational convenience only.
