...
## El Papel de las Variables Binarias {#S4:BinaryVar}

\index{variable explicativa!categórica}
\index{variable explicativa!factor}
\index{variable categórica}
...
\index{variable categórica!ficticia}
...
\index{variable categórica!no ordenada}
\index{símbolos!$c$, número de niveles en una variable categórica}
...
\index{datasets!seguro de vida temporal}
...
(ref:BoxFACEMARSTAT) **Diagramas de Caja del Logaritmo del Monto de Cobertura, por Nivel de Estado Civil** 


```{r BoxFACEMARSTAT, fig.cap='(ref:BoxFACEMARSTAT)', echo=FALSE, out.width="60%"}
library(HH)
...
```{r ref.label = 'MaritalSumStats', eval = FALSE}
```

```{r ref.label = 'BoxFACEMARSTAT', eval = FALSE}
```
...
```{r RandHIECoefficients, echo=FALSE}
tableout <- 
  data.frame(
...
### La Hipótesis Lineal General {#S4:GenLinHypo}
...
i. Comenzamos ejecutando un modelo de regresión con todas las $k+p=5$ variables. Los resultados se informaron en la Tabla \@ref(tab:MSTermLifeANOVA), donde vimos que $(Error~SS)_{full} = 615.62$ y $s_{full}^2 = (1.513)^2 = 2.289$.
...
Por ejemplo, en nuestro ejemplo de Seguro de Vida Temporal, probar la adecuación del modelo significa evaluar $H_0: \beta_1 = \beta_2 = \beta_3 = \beta_4 = \beta_5 = 0$. En la Tabla \@ref(tab:MSTermLifeANOVA), el cociente $F$ es 68.66 / 2.29 = 29.98. Con $df_1=5$ y $df_2 = 269$, el valor $F$ es aproximadamente 2.248 y el valor $p$ correspondiente es $\Pr(F > 29.98) \approx 0$. Esto nos lleva a rechazar firmemente la idea de que las variables explicativas no son útiles para entender la cobertura del seguro de vida temporal, reafirmando lo que aprendimos en el análisis gráfico y de correlación. Cualquier otro resultado sería sorprendente.
...
### Estimando y Prediciendo Varios Coeficientes {#S4:SetsInference}
...
Estos factores se resumen en la variable categórica de clase de riesgo CLASS. La Tabla \@ref(tab:AutoSumStats) muestra 18 clases de riesgo - no se proporciona información adicional de clasificación aquí para proteger los intereses propietarios de la aseguradora.


La Tabla \@ref(tab:AutoSumStats) resume los resultados de $n=6,773$
...
La Figura \@ref(fig:BoxplotAuto) muestra la distribución de las
reclamaciones logarítmicas por clase de riesgo.



```{r AutoSumStats, echo = FALSE}
...
(ref:BoxplotAuto) **Diagramas de Caja de Reclamaciones Logarítmicas por Clase de Riesgo** 

```{r BoxplotAuto, fig.cap='(ref:BoxplotAuto)', echo=FALSE, out.width="100%"}
library(HH)
AutoC   <- read.csv("CSVData/AutoClaims.csv", header=TRUE)
...
```{r ref.label = 'AutoSumStats', eval = FALSE}
```

```{r ref.label = 'BoxplotAuto', eval = FALSE}
```
...
\end{array}
$$
Las dos últimas igualdades se derivan de la manipulación algebraica. El $Factor ~SS$ desempeña el mismo papel que el $Regression ~SS$ en los Capítulos 2 y 3. La descomposición de la variabilidad se resume en la Tabla \@ref(tab:OneFactorANOVA).

```{r OneFactorANOVA, echo=FALSE}
library(knitr)
...
**Ejemplo: Reclamaciones de Automóviles - Continuación.** Para ilustrar, la tabla ANOVA que resume el ajuste para los datos de reclamaciones de automóviles se presenta en la Tabla \@ref(tab:ANOVAAuto). Aquí, vemos que la media cuadrática del error es $s^2 = 1.14.$

```{r ANOVAAuto, echo=FALSE}
library(knitr)
...
Para cada modelo presentado en la Tabla \@ref(tab:OneFactorCovariate),
...
Cada uno de los cinco modelos definidos en la Tabla \@ref(tab:OneFactorCovariate) fue ajustado a este subconjunto del estudio de caso hospitalario. Las estadísticas resumen se encuentran en la Tabla \@ref(tab:DRGModels). Para este conjunto de datos, hay $n = 79$ observaciones y $c = 3$ niveles del factor DRG. Para cada modelo, los grados de libertad del modelo son el número de parámetros del modelo menos uno. Los grados de libertad del error son el número de observaciones menos el número de parámetros del modelo.

Usando variables binarias, cada uno de los modelos en la Tabla \@ref(tab:OneFactorCovariate) puede escribirse en un formato de regresión. Como hemos visto en la Sección 4.2, cuando un modelo puede escribirse como un subconjunto de otro modelo más grande, tenemos procedimientos formales de prueba disponibles para decidir cuál modelo es más apropiado. Para ilustrar este procedimiento de prueba con nuestro ejemplo de DRG, a partir de la Tabla \@ref(tab:DRGModels) y los gráficos asociados, parece claro que el factor DRG es importante. Además, una prueba $t$, que no se presenta aquí, muestra que la covariable $x$ es importante. Por lo tanto, comparemos el modelo completo $\mathrm{E}~y_{ij} = \beta_{0,j} + \beta_{1,j}x$ con el modelo reducido $\mathrm{E}~y_{ij} = \beta_{0,j} + \beta_1x$. En otras palabras, ¿hay una pendiente diferente para cada DRG?

Usando la notación de la Sección 4.2, llamamos al intercepto y la pendiente variables el modelo completo. Bajo la hipótesis nula, $H_0: \beta_{1,1} = \beta_{1,2} = \beta_{1,3}$, obtenemos el modelo con intercepto variable y pendiente constante. Así, usando el cociente $F$ en la ecuación \@ref(eq:eq42), tenemos:
...
Aquí, las variables $x_1$, $x_2$, y $x_3$ se conocen como los *efectos principales*. La Tabla \@ref(tab:TwoFactorModel) ayuda a interpretar esta ecuación. Específicamente, hay seis tipos de personas que podríamos encontrar: hombres y mujeres que son jóvenes, de mediana edad o ancianos. Tenemos seis parámetros en la ecuación \@ref(eq:eq412). La Tabla \@ref(tab:TwoFactorModel) proporciona el enlace entre los parámetros y los tipos de personas. Al usar los términos de interacción, no imponemos ninguna especificación previa sobre los efectos aditivos de cada factor. De la Tabla \@ref(tab:TwoFactorModel), vemos que la interpretación de los coeficientes de regresión en la ecuación \@ref(eq:eq412) no es directa. Sin embargo, usar el modelo aditivo con términos de interacción es equivalente a crear una nueva variable categórica con seis niveles, uno para cada tipo de persona. Si los términos de interacción son críticos en su estudio, puede ser conveniente crear un nuevo factor que incorpore los términos de interacción simplemente para facilitar la interpretación.
...
b. Ahora introducimos la variable REGION, resumida en la Tabla 4.11. Un diagrama de caja de las expectativas de vida versus REGION se muestra en la Figura \@ref(fig:UNLIFEPlot3). Describe lo que aprendemos de la Tabla y el diagrama de caja sobre el efecto de REGION en LIFEEXP.

(ref:UNLIFEPlot3) **Diagramas de Caja de LIFEEXP por REGION** 

```{r UNLIFEPlot3, fig.cap='(ref:UNLIFEPlot3)', echo=FALSE, out.width="60%"}
   ## Figura 4.8
#  FIGURA 3.11
...
```
   ...
   La Tabla 4.11 resume la esperanza de vida promedio por región.

   | REGIÓN | Descripción de la Región                | Número | Media |
   |--------|-----------------------------------------|--------|-------|
   | 1      | Estados Árabes                          | 13     | 71.9  |
   | 2      | Asia Oriental y Pacífico                | 17     | 69.1  |
   | 3      | América Latina y el Caribe              | 25     | 72.8  |
   | 4      | Asia Meridional                         | 7      | 65.1  |
   | 5      | Europa Meridional                       | 3      | 67.4  |
   | 6      | África Subsahariana                      | 38     | 52.2  |
   | 7      | Europa Central y Oriental               | 24     | 71.6  |
   | 8      | OECD de Altos Ingresos                  | 23     | 79.6  |
   |        | Todos                                   | 150    | 67.4  |
...
**Modelo con una Variable Categórica.** Considera el modelo con una variable categórica introducido en la Sección 4.3 con $c$ niveles de la variable categórica. A partir de la ecuación \@ref(eq:eq49), este modelo se puede escribir como
...
(\#eq:eq422)
$$
donde $\boldsymbol \alpha _1 = \boldsymbol \beta _1 +
\mathbf{A}\boldsymbol \beta _2 $ es un nuevo vector de parámetros. La razón para introducir esta nueva parametrización es que ahora el vector de variables explicativas es *ortogonal* a las otras
...
(\#eq:eq426)
$$
Como se mencionó en la Sección 4.3, la cantidad $(Error ~SS)_{reduced} - (Error ~SS)_{full}$ se llama *suma extra de cuadrados*, o suma de cuadrados Tipo III. Esto se produce automáticamente por algunos paquetes de software estadístico, evitando así la necesidad de ejecutar regresiones separadas.

### Modelo Lineal General

Recuerda el modelo lineal general de la Sección 4.4. Es decir, usamos
$$
...
Como en la Sección 4.2, la estadística de prueba para examinar $H_0$: $\mathbf{C \boldsymbol \beta} = \mathbf{d}$ es
$$
