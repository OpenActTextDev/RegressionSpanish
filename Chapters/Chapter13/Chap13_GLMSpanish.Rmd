...
\begin{array}{lllrr}
\hline
\text{Categoría} & \text{Variable} & \text{Descripción} & \text{Porcentaje} & \text{Mediana de } \\
&  &  & \text{de datos} & \text{Gastos}\\ \hline
& COUNTIP& \text{Número de gastos (mediana: 1.0)} \\
\text{Demografía} & EDAD & \text{Edad en años entre}\\
& & \ \ \ \text{ 18 a 65 (mediana: 41.0) }\\
& GENERO &\text{1 si es mujer} & 72.0 & 5,546 \\
&  & \text{0 si es hombre} & 28.0 & 7,313 \\
\text{Etnicidad} & ASIATICO & \text{1 si es asiático} & 2.6 & 4,003 \\
& NEGRO & \text{1 si es negro} & 19.8 & 6,100\\
& NATIVO & \text{1 si es nativo} & 1.9 & 2,310\\
& BLANCO & \textit{Nivel de referencia} & 75.6 & 5,695\\
\text{Región} & NORDESTE & \text{1 si es del noreste} & 18.5 & 5,833 \\
& MEDIOESTE & \text{1 si es del medio oeste} & 21.7  &  7,999 \\
& SUR & \text{1 si es del sur} & 40.8  &  5,595 \\
& OESTE & \textit{Nivel de referencia} & 19.1  &  4,297\\ 
\hline \text{Educación} & UNIVERSIDAD
& \text{1 si tiene título universitario o superior} & 23.6 &  5,611\\
& \text{SECUNDARIA} & \text{1 si tiene título de secundaria} & 43.3 & 5,907  \\
&& \text{Nivel de referencia es menor} & 33.1  &  5,338 \\ 
&& \ \ \ \text{   que título de secundaria}    & \\ \hline
\text{Autoevaluación} & MALA & \text{1 si es mala} & 17.2 & 10,447\\
\ \ \text{salud física} & REGULAR & \text{1 si es regular} & 10.2 & 5,228 \\
\ \ \text{ } & BUENA & \text{1 si es buena }& 31.2 & 5,032\\
& VMUYBUENA & \text{1 si es muy buena} & 24.8 &  5,546 \\
&&   \text{Nivel de referencia es excelente salud} &16.6 & 5,277 \\
\text{Autoevaluación} & MMALA & \text{1 si es mala o regular} & 15.9  & 6,583  \\
\ \ \text{salud mental} &  & \text{0 si buena a excelente salud mental} & 84.1 & 5,599 \\
\text{Alguna limitación} & ALGLIMITE & \text{1 si tiene alguna limitación funcional/de actividad}&
41.4 & 7,826  \\
\ \ \text{de actividad} &  & \text{0 si no tiene limitación}& 58.6 & 4,746 \\ 
\hline \text{Ingresos} &&  \text{Nivel de referencia son ingresos altos} & 21.7 &  7,271 \\
\text{comparado con} & MINGRESO & \text{1 si es ingreso medio} & 26.8 &  5,851 \\
\text{la línea de pobreza} & LINGRESO & \text{1 si es ingreso bajo} & 16.6 & 6,909  \\
& NPPOBRE & \text{1 si es casi pobre} & 7.0 & 5,546 \\
& POBRONEG & \text{si es pobre/ingreso negativo} & 28.0 & 4,097 \\ 
\hline \text{Seguro} & SEGURO & \text{1 si está cubierto por seguro público/privado}
&91.1 &  5,943 \\
\ \ \text{médico} &  & \ \ \text{seguro en cualquier mes de 2003} &  &  \\
&  & \text{0 si no tiene seguro médico en 2003} & 8.9 & 2,668  \\ 
\hline Total &  &  & 100.0 & 5,695 \\ \hline
\end{array}
}
$$
...
\mathbf{x}_i^{\prime} \boldsymbol \beta = y_i - \mu_i = \varepsilon_i$, resultando en residuales crudos (ordinarios). Otra opción es reescalar mediante la desviación estándar
$$
...
\begin{array}{lcc}
\hline
\text{Distribución} & \text{Transformación Aproximada a la Normalidad} & \text{Transformación de Estabilización de Varianza} \\
 & \text{Transformación} & \text{Transformación} \\
