...
written as $\mathrm{Var~}y_i = \phi v(\mu_i)$. [Table 13.1](#Tab131) shows how the variance depends on the
mean for different distributions. As we will see when considering
estimation (Section \@ref(S:Sec133)), it is the choice of the
...
density or mass function, depending on the application. [Table 13.8](#Tab138) provides several examples, including the normal,
...
$\mathrm{E~}y=b^{\prime }(\theta )=\theta =\mu$ and $\mathrm{Var~} y = \sigma ^{2}b^{\prime \prime }(\theta )=\sigma ^{2}$, as
anticipated.
...
Table [Table 13.2](#Tab132) shows the mean function and corresponding
canonical link for several important distributions.

<a id=Tab132></a> 

[Table 13.2]: ./C13GLM.html#Tab132

[Table 13.2]{#Tab132}. **Mean Functions and Canonical Links for Selected Distributions** 
...
Links relate the mean to the systematic component and to
...
type. [Table 13.3](#Tab133) displays the Mildenhall data
...
[Table 13.3]: ./C13GLM.html#Tab133

[Table 13.3]{#Tab133}. **Private Passenger Automobile UK Collision Data** 
...
$$
*Source: Mildenhall (1999). "DriveShort" means drive to work but less than 10  miles."DriveLong" means drive to work but more than 10 miles.*

`r HideRCode('Tab133.Hide',"R Code to Produce Table 13.3")`
...
TableGen1(TableData=tableout, 
         TextTitle='Private Passenger Automobile UK Collision Data', 
...
As described in Section 11.9, maximum likelihood estimators are consistent and have large sample
...
variance is needed, see [Table 13.1](#Tab131). Unlike
...
data in [Table 13.3](#Tab133), a log-link function with
gamma distribution was fit using claim counts as weights ($w_i$).
[Table 13.4](#Tab134) shows estimates of the
...
[Table 13.4]: ./C13GLM.html#Tab134

[Table 13.4]{#Tab134}. **Estimated Expected Severity for a Multiplicative Rating Plan** 
...
`r HideRCode('Tab134.Hide',"R Code to Produce Table 13.4")`
...
colnames(tableout) <- c("Age Group", "DriveShort", "DriveLong", "Pleasure", "Business")
TableGen1(TableData=tableout, 
         TextTitle='Estimated Expected Severity for a Multiplicativefv Rating Plan', 
...
extra variability. For example, [Table 13.8](#Tab138) shows that
...
v(\mu_i)$ is the variance function (as in the  [Table 13.1](#Tab131) examples) and that $\widehat{\mu}_i$ is
...
\theta}) \right). 
$$ 
In linear exponential families, one multiplies by the scaling factor $\phi$ to define the
*deviance statistic*, $\mathbf{\mathrm{D}}(\widehat{\boldsymbol \theta})$ $= \phi \mathbf{\mathrm{D}}^{\ast}(\widehat{\boldsymbol \theta})$. This multiplication actually removes the variance
...
hospital during the year. [Table 13.5](#Tab135) summarizes
the data using the same explanatory variables as in Table 11.4. For
example, [Table 13.5](#Tab135) shows that the sample is 72\%
...
[Table 13.5](#Tab135) also gives median expenditures by
...
[Table 13.5]: ./C13GLM.html#Tab135

[Table 13.5]{#Tab135}. **Median Expenditures by Explanatory Variable  Based on a Sample of $n=157$ with Positive Expenditures** 
...
`r HideRCode('Tab135.Hide',"R Code to Produce Table 13.5")`

As noted in the preface, most of the code for the book, written about 2008, was in SAS. The illustrative code provided here, in `R`, exhibits some slight discrepancies from the original.
...
[Table 13.5](#Tab135) uses medians as opposed to means
because the distribution of expenditures is skewed to the right.
This is evident in Figure \@ref(fig:Fig131) that provides
...
(ref:Fig131) **Smooth Empirical Histogram of Positive Inpatient Expenditures.** The largest  expenditure is omitted.
...
#hist(Hexpend58$EXPENDIP, main="", xlab="Expenditures")

```

`r HideRCode('Fig131.Hide',"R Code to Produce Figure 13.1")`
...
inpatient expenditures using all explanatory variables. The results of this model fit appear in [Table 13.6](#Tab136). Here, we see that many of the potentially important determinants of
...
these categorical variables to binary variables. [Table 13.6](#Tab136), under the heading "Reduced Model,"
...
For another alternative model specification, [Table 13.6](#Tab136) also shows a fit of an inverse gaussian model with a logarithmic link. From the $AIC$ statistic, we see that
...
[Table 13.6]: ./C13GLM.html#Tab136

[Table 13.6]{#Tab136}. **Comparison of Gamma and Inverse Gaussian Regression Models** 
...
`r HideRCode('Tab136.Hide',"R Code to Produce Table 13.6")`
...
kable(tidy_model, format = "html", digits = 3, 
      ...
      caption = "Summary of the Gamma Regression Model")
...
is to choose the transform so that the variance is stabilized. [Table 13.7](#Tab137) gives transforms for the binomial,
Poisson and gamma distributions.


<a id=Tab137></a> 

[Table 13.7]: ./C13GLM.html#Tab137

[Table 13.7]{#Tab137}. **Approximate Normality and Variance-Stabilizing Transforms** 
...
$$
*Note*: $\mathrm{h}_B(u) = \int_0^u s^{-1/3} (1-s)^{-1/3}ds$ is the incomplete beta function evaluated at $(2/3, 2/3)$, up to a scalar constant. *Source: Pierce and Schafer (1986)*. 
...
(\#eq:eq1312)
\end{equation}
where $1<p<2.$ Examining our variance function [Table 13.1](#Tab131), the Tweedie distribution can also be
...
13.1 Verify the entries in [Table 13.8](#Tab138) for the gamma
...
classification example using data in [Table 13.3](#Tab133).
...
estimated expected severities in [Table 13.4](#Tab134).
...
function, depending on the application. [Table 13.8](#Tab138)
...
[Table 13.8]: ./C13GLM.html#Tab138

[Table 13.8]{#Tab138}. **Selected Distributions of the One-Parameter Exponential Family** 
...
$$
$^{\ast}$ This assumes that the parameter $r$ is fixed but need not be an integer.
...
b(\theta)}{\phi} \right).
\end{eqnarray*}
This yields $\mathrm{E}y^2 = M^{\prime \prime}(0)$ $= \phi b^{\prime \prime}(\theta) + (b^{\prime}(\theta))^2$ and $\mathrm{Var}~y =\phi b^{\prime \prime}(\theta)$.

### Maximum Likelihood Estimation for General Links {#S:Sec1393}

For general links, we no longer assume the relationship $\theta_i=\mathbf{x}_i^{\mathbf{\prime }}\boldsymbol \beta$ but assume that
